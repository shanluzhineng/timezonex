...
func main() {
	...
	tmpl, err := template.New("gen").Funcs(funcMap).Parse(output)
	if err != nil {
...
var (
	...
	zoneMap  map[string]interface{} = make(map[string]interface{})
	zoneList []string
)
...
func init() {
	...
	once.Do(func() {
		...
		for i := 0; i < len(countries); i++ {
			...
			for _, eachZone := range countries[i].Zones {
				if _, ok := zoneMap[eachZone.Name]; ok {
					continue
				}
				zoneMap[eachZone.Name] = true
				zoneList = append(zoneList, eachZone.Name)
			}
		}
	})
}
